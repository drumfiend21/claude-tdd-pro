...
**12. Commit spec patches in the SAME CL that introduces them. ALWAYS `git add evals/specs/` before commit.** When you patch an `evals/pending/...json` spec to fix a stream-redirect, hardcoded path, or content bug — the same patch lives in `evals/specs/` after promotion, and that location is what HEAD reads. **MANDATORY pre-commit step**: include `git add evals/specs/` (the entire directory) on every implementation CL. Yes, this risks staging unintended changes — but the alternative (silently shipping a broken HEAD that the active suite somehow keeps passing) is worse. Verify post-commit with: `git stash push -- "evals/specs/<file>" && bash evals/runner.sh -v <spec-name>`; if HEAD's form fails the same spec that WT's form passes, you missed the add. **Why:** the gap fired TWICE — CL-100 caught it in CL-93/94/97 (13 specs), and CL-117 caught it AGAIN in CL-114/115/116/117 (~28 specs) despite item 12 being persisted. The lesson is that "scan git status before committing" is too easy to skip; the only reliable defense is mechanically `git add evals/specs/` every time. **How to apply:** every `git commit` line for an implementation CL must list `evals/specs/` (or `evals/specs/*.json`) in its `git add` argument list, even if you didn't intentionally edit anything in there.
