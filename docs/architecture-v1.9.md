...
## §33. `standards-refresh.sh` — the ADR-0009 line 50 registry-walker orchestrator (v1.26 amendment — GCTP KA-5 P-21 trigger)

**STATUS: BUILT (2026-07-11).** Closes the URL-registry → periodically re-scraped, 4-axis-tagged, aggregated rules seam by **activating existing infrastructure**, not inventing new architecture (Option 3 hybrid). GCTP KA-5 (P-21) surfaced the miss: registries with 90 declared URLs across 7 files, target namespaces empty, no orchestrator wiring the schedulers (C-19 / S-17 `--upstream-stub` placeholders) to the six-stage ADR-0009 pipeline. Detail: [docs/design/v1.26-standards-refresh-registry-walker.md](design/v1.26-standards-refresh-registry-walker.md).

**Two connected gaps, both pre-committed by the architecture but not built:** (1) ADR-0009 line 50 names `standards-refresh.sh` as the driver that "scrapes source URLs" — no such script existed. (2) ADR-0009 line 51 promises five Stage-1 shapes (markdown-headings, html-sections, numbered-list, free-prose, pdf-sections); only 2 shipped in `extract-rules-from-url.sh`. Every registered URL sat on the shelf.

- **`commands/standards-refresh.sh`** — the registry-walker orchestrator per ADR-0009 line 50. Single mode: `--registry <path> [--force] [--now <iso>] [--dry-run] [--out-dir <dir>] [--replace]`. Walks each entry, freshness-skip via `<registry>-last-fetch/<source-id>.txt` vs entry `fetch_frequency`, dispatches through `standards/fetcher.sh` (S-2) using `standards/fetchers/http-get.sh` as the generic upstream stub (honors §2.6 `fragility_tier` + `fragility_strategy`; cache preserved on upstream failure per §16 S-2), feeds cached content into Stage 1 with shape inferred from the entry `fetcher:` field, runs Stages 2+3+4 (classify + route + auto-bind `architectural-content` bundle on `applies_to_prose:true`), assembles YAML with §28.40 `introduced_in` epoch tag on every rule, merges idempotently by `content_hash` (existing rules preserved; new rules appended with `introduced_in: <NOW>`; removed rules marked `deprecated: true` + `deprecated_at: <NOW>` + `deprecated_reason: removed-upstream`), atomically writes to `<out-dir>/<resolved-namespace>/<framework-id>.yaml` per §17 G-9 namespace resolution, updates last-fetch marker.
- **`commands/extract-rules-from-url.sh` extended to all 5 ADR-0009 line 51 shapes.** Adds `html-sections` (scans `<h2..h6>` tags; body is HTML-stripped prose until next same-or-higher heading; `<h1>` = document title), `free-prose` (blank-line-delimited paragraphs; title = first policy-verb sentence MUST/SHALL/MAY/SHOULD/REQUIRED or first 8 words), `pdf-sections` (pdftotext-style text with section labels `Section 1.2` / `V14.3` / `§5.1` / `Article 5` / `Art. 6`). Existing `markdown-headings` + `numbered-list` byte-identical. Same JSON output shape.
- **`standards/fetchers/http-get.sh`** — generic upstream stub for the S-2 dispatch. Reads `URL` env, curls with `--max-time` (default 20s), emits stdout. Non-zero exit on failure so `standards/fetcher.sh` preserves the prior cache per §16 S-2 discipline. Enables fragility-tier + cache preservation for any registered URL without per-source stubs.
- **Shape inference (registry-walker table):** `html-anchor.sh` → `html-sections`; `markdown-headers.sh` → `markdown-headings`; `pdf-section.sh` → `pdf-sections`; `rfc-style.sh` → `free-prose`; other/absent → `markdown-headings`. Uses the `fetcher:` field every registry entry already declares (§2.6 / §2.19 / §2.12); no new registry schema.
- **Namespace inference chain (per §17 G-9 + §2.6):** explicit `source_namespace:` on the entry → else `applies_to[0]` from the entry's list (the operator-facing field defined in §2.6 for standards sources) → else jurisdiction-mapped from §17 G-9 (compliance-side) → else `_universal`. Uses fields entries already declare; no new registry schema.
- **§28.40 grandfathering.** Existing rules whose `content_hash = sha256(title + "\n" + prose)` matches on re-run keep their original `introduced_in` (per Consumer Compatibility Contract). New rules take `introduced_in: <NOW>`. Removed rules keep their block but are marked deprecated with a `removed-upstream` reason. Re-run against unchanged URL → byte-identical YAML modulo `fetched_at` timestamp.
- **§25 fidelity vocabulary additions:** `standards-refresh`, `registry-walker`, `html-sections`, `free-prose`, `pdf-sections`, `shape-inferred`, `upstream-stub-generic`, `fragility-tier-dispatch`, `freshness-window`, `content-hash-preserved`, `deprecated-at`, `deprecated-reason`, `deprecated_reason`, `removed-upstream`, `http-get`, `six-stage-pipeline`, `option-3-hybrid`, `applies_to_first`, `namespace-inference-chain`, `4-axis-tag`, `foss-routing`, `end-to-end-chain`.
- **Anti-drift folder map:** `commands/standards-refresh.sh` (registry-walker) · `commands/extract-rules-from-url.sh` (Stage 1, now 5 shapes) · `standards/fetchers/http-get.sh` (generic upstream stub). Pending specs at `evals/pending/C/26-standards-refresh-registry-walker/`.

15 specs (`cl564-refresh-01..15`): html-sections-yields-segments / free-prose-yields-segments / pdf-sections-yields-segments / markdown-headings-regression / http-get-fetches-url / orchestrator-walks-registry / freshness-skip-honored / force-overrides-skip / content-hash-preserves-introduced-in / deprecated-at-on-removal / emitted-rules-carry-4-axis-tag / emitted-rules-routed-to-foss-tools / aggregator-picks-up-emitted-rules / write-time-enforcement-applies-emitted-rules / yield-per-url-many-rules. **§20 note:** completes ADR-0009 line 50 (orchestrator) + line 51 (5 shapes); activates the previously-unwired S-2 dispatch layer as the fetch precursor to Stage 1; preserves §28.40 grandfathering. **Empirical yield (2026-07-11):** 17 real URLs in `standards/sources.yaml` → 618 rules extracted (was 0 before this CL); every rule carries `applies_to` 4-axis tag from Stage 2, `enforced_by[]` FOSS tools from Stages 3+4, and `introduced_in` epoch tag per §28.40. Composes forward with CL-565 scheduler wire-through (one-line replacement of `--upstream-stub` in C-19/S-17) and CL-566 LLM Tier-2 integration (Wave-2 per ADR-0009). Preserves §21 dod. Suite 5041→5056.

**CL-564.b (§33 amendment, 2026-07-12) — Stage 5 wire-through: four-layer fidelity per emitted rule.** `commands/standards-refresh.sh` now runs ADR-0009 Stage 5 (`commands/draft-custom-rule.sh`, §28.34) per emitted rule against the rule's first routed tool (default `semgrep` when routing yields no tool; Layer D makes tool choice coverage-affecting only — the semantic moat holds regardless). Per-rule audit trail written into the emitted YAML as a `fidelity:` block (`clauses_total` / `clauses_covered` / `clauses_fallback` / `clauses_unenforceable` / `no_clause_dropped` / `needs_operator_signoff`); `prose-judge.sh` merged into `enforced_by` whenever `clauses_fallback > 0` (§28.34 Layer D binding). THE CONTRACT: any rule with `no_clause_dropped: false` — or Stage 5 unavailable/crashed (fail closed; the assertion is never silently skipped) — emits `FIDELITY-VIOLATION` on stderr and the source's file is REFUSED (entry fails, exit 1, nothing written). Per-entry stderr summary extended with `stage5=ok clauses_total=<n> covered=<c> fallback=<f> unenforceable=<u>` aggregate counters. Test affordance: `CTP_STAGE5_DRAFT_CMD` overrides the Stage 5 script path (same CLI as `draft-custom-rule.sh`) so specs can exercise the refusal path with a dropping-drafter stub. §25 fidelity vocabulary additions: `stage5`, `fidelity-audit-trail`, `fidelity-audit-trail-per-rule`, `fallback-binds-prose-judge`, `clause-drop-refuses-write`, `stage5-counters-on-stderr`, `FIDELITY-VIOLATION`, `CTP_STAGE5_DRAFT_CMD`, `dropping-drafter`, `stage5-wire-through`. 4 specs (`cl564-refresh-16..19`): fidelity-audit-trail-per-rule · fallback-binds-prose-judge · clause-drop-refuses-write · stage5-counters-on-stderr. Suite 5056→5060.

**CL-564.c (§33 amendment, 2026-07-12) — Stage 6 wire-through: `--gate review-queue` opt-in staging.** `commands/standards-refresh.sh` gains `--gate review-queue` (§28.36; the only supported `--gate` mode — any other value is a usage error). Gated: the source's YAML stages under `<out-dir>/_project/<crawl-id>/<resolved-namespace>/<framework-id>.yaml` instead of the official namespace; per-rule Stage 5 draft JSONs persist at `_project/<crawl-id>/drafts/<framework-id>/<rule-id>.json`; `review-queue.sh --dir` routes them (auto-stage / coverage-review / side-by-side-review) with NO `--auto-accept` passed — default stays human-in-the-loop per §28.36, `staged=0`, nothing reaches the official namespace until the operator reviews and promotes. Ungated: byte-identical default auto-write (no `_project/` dir ever created). `<crawl-id>` = registry basename (sans extension) + `-` + colon-free run timestamp (deterministic under `--now`). Gated runs still update last-fetch markers (the fetch happened); `--force` re-runs ungated inside a freshness window. `--dry-run` composes but writes neither YAML nor drafts. §25 fidelity vocabulary additions: `--gate`, `gate`, `crawl-id`, `crawl`, `_project`, `STAGED`, `promote-after-review`, `gate-stages-under-project-crawl`, `gate-routes-drafts-hitl-default`, `ungated-default-auto-write-unchanged`, `unknown-gate-mode-usage-error`, `everything-goes`, `hitl`. 4 specs (`cl564-refresh-20..23`): gate-stages-under-project-crawl · gate-routes-drafts-hitl-default · ungated-default-auto-write-unchanged · unknown-gate-mode-usage-error. Suite 5060→5064.

**CL-564.d (§33 amendment, 2026-07-12) — prose-shape sources yield prose rules.** Entries whose `fetcher:` is one of `html-anchor.sh` / `markdown-headers.sh` / `rfc-style.sh` produce rules extracted from published PROSE guidance, so `commands/standards-refresh.sh` forces `applies_to_prose: true` on every rule from such a source BEFORE Stage 4 routing — the router then auto-attaches the `architectural-content` bundle unconditionally (§28.30 / ADR-0009 stage 4) and the rules fire on ADRs and design prose at design time (§29.4 same-engine enforcement). `pdf-section.sh` and all other fetchers keep the classifier-derived flag untouched (document-shaped sources are not presumed prose-applicable). §25 fidelity vocabulary additions: `prose-shape`, `prose-shape-sources`, `html-source-yields-prose-rules-with-bundle`, `rfc-style-source-yields-prose-rules`, `markdown-source-yields-prose-rules`, `pdf-source-keeps-classifier-flag`, `classifier-derived`, `design-time-firing`, `bundle` (the emitted `enforced_by[]` YAML key, `bundle: architectural-content`). 4 specs (`cl564-refresh-24..27`): html-source-yields-prose-rules-with-bundle · rfc-style-source-yields-prose-rules · markdown-source-yields-prose-rules · pdf-source-keeps-classifier-flag. Suite 5064→5068.

**CL-564.e (§33 amendment, 2026-07-12) — per-source acquisition-sufficiency signal (§31.9 A9 pattern).** `commands/standards-refresh.sh` emits, per source, `standards-refresh: sufficiency sid=<id> rule_count=<n> sufficiency=ok|below-threshold-<N>` on stderr (`--threshold`, default 30, non-negative integer — non-numeric is a usage error) — mirroring the §31.9 sufficiency-signal contract: a source below the floor fails loud, never silent (signal only; the file is still written — the consuming harness owns the enforcement gate, per §31.9 "CTP owns the capability; GCTP owns the enforcement gate"). A source whose fetch yields ZERO usable rules is REFUSED outright: `INSUFFICIENT sid=<id> rule_count=0` on stderr, entry fails (exit 1), nothing written — and a target file written by a prior sufficient fetch survives byte-identical (composes with the S-2 cache-preservation discipline: a medium-fragility `prompt-on-change` source declines a >5% structure delta under `--no-confirm-default`, so an emptied upstream only reaches extraction at all via `silent-replace`). §25 fidelity vocabulary additions: `--threshold`, `threshold`, `INSUFFICIENT`, `below-floor-fails-loud-still-writes`, `configurable-floor-marks-sufficient`, `zero-rules-refuses-write`, `zero-rule-refetch-preserves-file`, `refuses-to-write`, `upstream-page-emptied`. 4 specs (`cl564-refresh-28..31`): below-floor-fails-loud-still-writes · configurable-floor-marks-sufficient · zero-rules-refuses-write · zero-rule-refetch-preserves-file. Suite 5068→5072.

**CL-564.f (§33 amendment, 2026-07-12) — domain-crawl: one registry entry, one bounded guarded crawl.** Operator-approved (item 5 of the GCTP handoff roadmap, proposed defaults). A registry entry MAY declare a `scope:` block (`strategy: single-page|path-prefix|sitemap|link-follow`, `max_pages`, `max_depth`, `respect_robots`, `rate_limit_ms`, `include_patterns`, `exclude_patterns`); non-single-page strategies expand the S-2-fetched seed through the new `commands/domain-crawl.sh`, which runs Stage 1 per fetched page and emits merged, content_hash-deduped segments — the downstream pipeline (classify → route → prose-forcing → Stage 5 fidelity → §31.9 sufficiency → assemble → §28.40 merge/deprecate → atomic write) is unchanged and applies to crawled sources automatically. **8 universal guards, always on (entry values only tighten):** same-host; `max_pages` hard cap 200; `max_depth` hard cap 3; robots.txt (`User-agent: *` Disallow, http/https); content-type filter (binary-extension excludes + NUL sniff); content_hash dedupe; rate limit (`rate_limit_ms` default 1000, `file://` exempt); URL length ≤ 2048. **Refuse-on-root-path:** `path-prefix` with a root prefix exits 2 — a whole-domain crawl must be an explicit `link-follow` choice, never an accident. **Quality guards:** empty-content-density gate (<5 tag-stripped words); low-yield pages recorded not failed; default `exclude_patterns` (login/signup/signin/register/search/mailto/javascript + static assets); `confidence_density=<n.nn>` on the per-entry pipeline summary; redirects delegated to the `http-get.sh` stub (link expansion can never leave the host); per-refresh crawl manifest at `.claude-tdd-pro/crawls/<source-id>-<ts>.jsonl` (one JSONL row per URL considered: url/depth/status/reason/content_hash/segments; `status: index` marks a sitemap index, never extracted). `scope:` is schema-additive; entries without it stay single-page byte-identical. Detail: [docs/design/v1.27-standards-refresh-domain-crawl.md](design/v1.27-standards-refresh-domain-crawl.md). §25 fidelity vocabulary additions: `domain-crawl`, `scope`, `strategy`, `path-prefix`, `sitemap`, `link-follow`, `single-page`, `max_pages`, `max_depth`, `respect_robots`, `rate_limit_ms`, `include_patterns`, `exclude_patterns`, `off-host`, `outside-prefix`, `max-pages`, `max-depth`, `duplicate-content`, `url-too-long`, `empty-content`, `low-yield`, `binary-content`, `robots-disallow`, `excluded-pattern`, `not-included`, `fetch-failed`, `REFUSED`, `refuse-on-root-path`, `crawl-manifest`, `crawls`, `pages_fetched`, `pages_skipped`, `confidence_density`, `crawl-merges-linked-pages`, `outside-prefix-links-skipped`, `off-host-links-never-followed`, `max-pages-budget-honored`, `max-depth-bound-honored`, `duplicate-content-processed-once`, `root-path-crawl-refused`, `crawl-manifest-records-trail`, `sitemap-fetches-mapped-leaves-only`, `no-scope-stays-single-page`, `loc`, `urlset`, `mirrored-retention-rule`, `evil`, `exfil`. 10 specs (`cl564-refresh-32..41`). Suite 5072→5082.

**CL-564.g (§31.9 S-60 registry addition, 2026-07-12) — Node.js canonical source registered.** Operator choice (a) (item 6 of the GCTP handoff roadmap): `standards/technology-source-registry.yaml` gains `node → nodejs-api-docs` (`https://nodejs.org/api/`, tier 1, `whole_source: true`) with a `scope: { strategy: path-prefix }` §33 domain-crawl annotation — ONE canonical seed whose crawl captures every module page under `nodejs.org/api/` automatically (child_process, fs, http, net, cluster, worker_threads, stream, events, crypto, and every future module) with zero registry edits when upstream adds pages. The `scope:` key is schema-additive per §28.40: crawl-capable consumers (the §33 orchestrator via `commands/domain-crawl.sh`) honor it; pre-crawl consumers (`acquire-technology-live.sh`) ignore it and fetch the seed page only — verified against the S-60 extraction shape. PR-gated per §31.2/§31.9 (this branch IS the reviewed PR). §25 fidelity vocabulary additions: `node-canonical-seed-registered`, `scope-invisible-to-pre-crawl-consumers`, `nodejs-api-docs`, `pre-crawl`, `dig`. 2 specs (`cl564-refresh-42..43`): node-canonical-seed-registered · scope-invisible-to-pre-crawl-consumers. Suite 5082→5084.

**CL-564.h (§33 amendment, 2026-07-12) — full-chain integration coverage + numeric-interval defect fix.** Operator-directed: the scrape → count → 4-axis-tag → enforcement-alignment → refresh chain is now integration-tested end-to-end for sources that exist now AND sources added later, at any declared interval. (1) **Multi-source counting:** a registry of several URLs across namespaces has every parsed rule counted per source (`rule_count=`) and filed in its namespace, with the cross-file total asserted. (2) **URLs added later:** an entry appended to a registry after prior refreshes is scraped, 4-axis-tagged, Stage-5-verified, and routed on the very next run while still-fresh entries `SKIP` — no re-registration, no `--force`. (3) **Interval-driven re-scrape:** a source declaring a numeric `fetch_frequency` (`30m`) is skipped inside its window and re-scraped + re-parsed purely by clock once the window elapses, picking up upstream additions with §28.40 grandfathering (original `introduced_in` preserved; new rule takes the re-scrape epoch). **Defect found + fixed by this spec:** `freq_to_seconds` had two stacked bash defects — `%` (shortest-suffix) matched the empty suffix, and `$n` inside a combined `local n=… unit=${freq#$n}` expanded before assignment — so every numeric interval (`30m`/`6h`/`1d`/`2w`/`1mo`) silently fell back to daily; keyword frequencies masked it. Fixed with `%%` + split assignments; all 8 formats unit-verified. (4) **Enforcement alignment:** every emitted rule aligns with an enforcement surface — a routed third-party tool (`checkov`/`trivy`/…), the semantic native fallback (`prose-judge.sh`, §28.34 Layer D / §28.57), the `architectural-content` bundle, or an explicit `needs_operator_signoff: true` — asserted per rule block, no rule left unaligned. (5) **Audit/detection-time:** walking an app tree file-by-file through the §29.6 shared enforcement primitive (`rubric/enforce-file.sh --root <emitted>`, the same engine `composite-audit.sh` drives) exercises every scraped rule against every applicable file with `not_enforced=0` — native enforcement covers absent third-party tools at detection time; write-time coverage held by the pre-existing write-time-enforcement spec. §25 fidelity vocabulary additions: `every-parsed-rule-counted-and-filed`, `later-added-url-flows-full-chain`, `interval-driven-rescrape-reparses`, `every-rule-enforcement-aligned`, `audit-detection-covers-every-scraped-rule`, `rules_checked`, `not_enforced`, `rotate-credentials-quarterly`, `grandfathered`, `unaligned`, `retain-operational-records`, `module-style-preference`, `encrypt-s3-buckets`, `audit-time`, `detection-time`, `blocks` (per-rule YAML block scan). 5 specs (`cl564-refresh-44..48`). Suite 5084→5089.

**CL-564.i (§33 amendment, 2026-07-12) — real-URL yield verification + two real-world-HTML defect fixes.** Operator-directed audit ("rule counts look low — confirm scraping/parsing/tagging works"): extraction verified EXACT against ground truth on live `nodejs.org/api/` pages — independent h2–h6 counts vs extracted segments per fetched page: index 1/1, documentation.html 6/6, synopsis.html 3/3, assert.html 30/30; a `max_pages: 4`-bounded path-prefix crawl (209 URLs considered) yielded 40 rules, `sufficiency=ok`, 2,567 clauses through Stage 5 with zero dropped, `confidence_density=0.88`, all 40 rules 4-axis tagged + enforcement-aligned. The prior low single-page yield (`node-docs` → 1 rule) matches the page's actual content (one `<h2>`) — the §33 CL-564.f crawl scope is the designed remedy, not an extractor defect. **Two real defects found by the live run, both spec-fixed:** (1) `domain-crawl.sh` link extraction matched only QUOTED `href` values — minified real-world HTML (the nodejs.org index carries 95 module links, all unquoted) yielded zero followed links; fixed to match quoted and unquoted forms (`cl564-refresh-49`, red→green). (2) Extracted segments passed to the assemble stage as an env var died at the kernel per-argument exec limit (~128KB, rc=126) on documentation-scale yields; segments now travel via a temp file (`cl564-refresh-50`, red→green, 80-rule × long-prose fixture). Sandbox bound recorded honestly: the verification environment's egress proxy 403s most registry hosts (google.github.io, semver.org, owasp.org, w3.org, …) — those fetches SKIP with cache preserved per §16 S-2 (environment policy, not a scraper defect; `owasp-asvs`/`owasp-top10`/`wcag-2-2`/`slsa`/`semver` additionally refuse `--auto` by design at tier-low `manual-only`). Full-registry yield on an unrestricted host remains the CL-564 v2 empirical record (17 URLs → 618 rules). §25 fidelity vocabulary additions: `unquoted-hrefs-followed`, `large-yield-assembles-clean`, `minified`, `unquoted`, `exec-limit`, `ground-truth`, `egress`. 2 specs (`cl564-refresh-49..50`). Suite 5089→5091.

**CL-564.j (§33 amendment, 2026-07-12) — cloud scrape plane: scheduled refresh workflow, PR-gated.** Operator-directed ("this should work in the cloud the way it works on a desktop"): `.github/workflows/standards-refresh-cloud.yml` runs the §33 registry-walker on GitHub's runners — which have open egress — daily (`0 6 * * *`, per-entry freshness windows still apply) and on `workflow_dispatch` (manual trigger from any device, with `force` + `registry` inputs). This separates the **scrape plane** (one polite scheduled fetch where the network is open) from the **consume plane** (every session — cloud sandbox, phone-driven, desktop, GCTP pin — reads current rules FROM GIT with zero egress needed at session time; sessions without network keep the last committed rules per the §16 S-2 cache-preservation discipline, sessions with network can still refresh live). Review-first per §31.2 (official ruleset is PR-gated): refreshed YAMLs push to a `standards-refresh/<ts>` branch and open a pull request — the workflow NEVER commits to `main`; no-change runs exit quietly. Tier-low `manual-only` sources (owasp-asvs, owasp-top10, wcag-2-2, slsa, semver) refuse `--auto` by design and remain operator-run. Runner env pins `LC_ALL=C.UTF-8` (inline `ruby -e` under a C/ASCII locale fails to parse UTF-8 — the same environment defect observed in restricted sandboxes). Note: a workflow's schedule activates only once the file exists on the default branch — landing on the CL branch is itself review-first. §25 fidelity vocabulary additions: `standards-refresh-cloud`, `scrape-plane`, `consume-plane`, `workflow_dispatch`, `cron`, `cloud-refresh-scheduled-and-dispatchable`, `cloud-refresh-stays-pr-gated`, `pr-gated`, `dispatchable`, `aliases`. 2 specs (`cl564-refresh-51..52`). Suite 5091→5093.

**CL-564.k (§33 amendment, 2026-07-12) — domain clearing: the egress allowlist is DERIVED from the registries.** Operator-directed ("the domain of any URL I add as a rules source must be cleared for scraping, cloud or local"): `standards/registry-domains.sh` walks every URL-registry surface (`standards/*sources*.yaml`, `standards/technology-*-registry.yaml`, `.claude-tdd-pro/*URLS*.yaml`, `pr-corpus/*SOURCES*.yaml` + `*sources*.yaml` — GLOBBED, so a registry file added later is swept automatically), extracts every `http(s)` URL, and emits the sorted unique domain set to the COMMITTED `standards/EGRESS-ALLOWLIST.txt` (currently 11 registries → 56 domains). Adding a URL to any registry surfaces its domain with zero manual list maintenance; `--check` is the drift gate (exit 1 + diff when a registry gained a domain the committed allowlist lacks) and runs as a standing step in the CL-564.j cloud-refresh workflow. Honest boundary, stated not papered over: a restricted cloud session's network allowlist is a **security setting only the operator can change** — code cannot grant itself egress. The allowlist artifact makes that operator step one paste, kept current by the gate; desktops and the GitHub Actions scrape plane have open egress and need nothing. `--root` supports scanning an alternate plugin root (consumer surface, also the spec seam). §25 fidelity vocabulary additions: `registry-domains`, `EGRESS-ALLOWLIST`, `allowlist`, `drift-gate`, `drift=none`, `drift=DRIFT`, `committed-allowlist-in-sync`, `added-url-domain-surfaces-automatically`, `drift-gate-red-on-uncleared-domain`, `uncleared`, `netloc`, `cleared`. 3 specs (`cl564-refresh-53..55`). Suite 5093→5096.

**CL-564.l (§33 amendment, 2026-07-12) — project-scoped source registries: plugin-as-library (the ESLint model).** Operator-directed ("users add their own URLs in their own projects — like a third-party library — never in my repos; no CI triggered by them"): `commands/standards-refresh.sh --project <id>` routes emitted rules to the §31.1/S-63 per-project working overlay `<out-dir>/_project/<id>/<namespace>/<source-id>.yaml` instead of the official namespaces. A consumer keeps their OWN registry YAML in their OWN repo, refreshes into their own overlay (their `--out-dir`), and consumes with `rubric/aggregator.sh --root <their rules> --project <id>` — S-63 blast-radius scoping means the rule set aggregates ONLY under that project scope and the official surface stays byte-identical without it. The plugin author's repos are never touched: no branch, no PR, no workflow run. Project-id validation refuses path-unsafe ids (`[A-Za-z0-9._-]` only; `.`/`..` explicitly rejected — a dot-dot id would traverse the overlay write into the official namespaces). `--gate review-queue` takes precedence over `--project` when both are given (staging is upstream of any overlay). **Workflow-isolation facts recorded (operator concern):** `schedule` triggers execute ONLY from the hosting repo's default branch (a workflow on a CL/dev branch is inert until merged); `workflow_dispatch` requires WRITE permission on the hosting repo (plugin users have none); workflows execute only in the repo that hosts them — plugin consumers (e.g. a pinned plugin cache) receive files, never CI execution; GitHub additionally disables scheduled workflows on forks by default. Contributing a source to the OFFICIAL registries remains exactly what the operator specified: a dev branch + reviewed PR to the plugin repo (§31.2). §25 fidelity vocabulary additions: `--project`, `plugin-as-library`, `eslint-model`, `project-scope-routes-to-overlay`, `project-rules-scoped-to-their-project`, `unsafe-project-id-rejected`, `dot-dot-project-id-rejected`, `MY-SOURCES`, `my-team-standards`, `blast-radius`, `traverse`, `dot-dot`, `escape`. 4 specs (`cl564-refresh-56..59`). Suite 5096→5100.

**CL-564.m (§33 amendment, 2026-07-12) — config-declared sources: the npm/ESLint consumption surface.** Operator-directed ("distribution like a node module — sealed, not for tampering; it exposes something to the consumer's repo for registering URLs, like an import + register call"): `commands/standards-refresh.sh --config <ctp.config.yaml|dir>` reads a `sources:` list from the consumer's OWN §28.48 single config file (same entry fields as a registry, `scope:` included, real-YAML-parsed via ruby since the config also carries rule overrides) plus an optional `project:` overlay id (default: the config directory's sanitized name), synthesizes a registry, and refreshes into the §31.1/S-63 project overlay. Registering a rules-source URL is therefore ONE DECLARATIVE BLOCK in the consumer's own repo-root config — the npm-module analog maps as: plugin distribution (pinned cache / plugin install, lockfile-pinned, never edited) ⟷ `node_modules/`; `ctp.config.yaml` at the consumer repo root ⟷ `.eslintrc`; `sources:` ⟷ `plugins:`/`extends:` registration; `--config` refresh + `aggregator.sh --project` ⟷ `npx eslint .`. Declarative-over-callback is deliberate: the consumer repo registers DATA only — no consumer code ever executes inside the engine (composes with the agent-operating-compact and the EO security posture). `--registry` and `--config` are mutually exclusive; a config with no `sources:` runs clean (`total=0`); the synthesized registry basename carries `sources` so last-fetch markers land in the standard `standards-last-fetch` dir keyed per source id. §25 fidelity vocabulary additions: `--config`, `ctp-config-sources`, `config-sources-refresh-into-named-overlay`, `overlay-defaults-to-directory-name`, `empty-sources-config-runs-clean`, `registry-config-exclusive`, `mutually-exclusive`, `declarative`, `acme-storefront`, `acme-eng-standards`, `solo-src`, `npm-module`, `node_modules`. 4 specs (`cl564-refresh-60..63`). Suite 5100→5104.

**CL-564.n (§33 amendment, 2026-07-12) — disclosed public-only source-URL telemetry.** Operator-directed (learn which authoritative sources the fleet uses; report all PUBLIC-domain URLs, IGNORE anything not public to avoid infringing/​stealing others' property; stay license-clean + commercially sellable; disclose it, no notification noise). `commands/source-url-telemetry.sh` reports registered PUBLIC source URLs to the plugin author. **Public-only, resolution-gated:** a URL is reported only if its host resolves to a globally-routable address; suppressed entirely (never transmitted, never logged) are RFC1918/loopback/link-local IPs, reserved TLDs (`.internal`/`.local`/`.localhost`/`.test`/`.example`/`.invalid`/`.home.arpa`/`.corp`/`.lan`/`.intranet`), single-label intranet hosts, non-`http(s)` schemes (`file://`), and any host that resolves private or is unresolvable (internal-only DNS). Only `scheme://host/path` is sent — query + fragment stripped (may carry secrets); no repo content/path/user. **Disclosed + opt-out:** on by default, one-line stderr notice each time a public URL is actually shared; opt out via `CTP_TELEMETRY=off`, the cross-tool `DO_NOT_TRACK=1` standard, or `telemetry: off` in `ctp.config.yaml` (YAML-1.1 `off` boolean handled). **No GitHub noise:** a quiet HTTPS POST — never an issue/PR/comment/notification on any repo. **Fails open:** unreachable endpoint ⇒ silent no-op, never blocks a refresh. Wired into `standards-refresh.sh` per processed entry (best-effort, non-fatal); `file://` sources classify suppressed so the pre-existing 63 specs are byte-unaffected. **Docs made truthful (the conflict this resolved):** `SECURITY.md` previously said "no upload, no telemetry beacon" — rewritten to scope that promise to the local-only SPACE data and to fully disclose this feature; new `docs/telemetry.md` is the complete disclosure. Test seam: `CTP_TELEMETRY_TRANSPORT` replaces the POST, `CTP_TELEMETRY_RESOLVED=global|private|none` forces resolution — specs stay hermetic. §25 fidelity vocabulary additions: `source-url-telemetry`, `telemetry`, `public-only`, `resolution-gated`, `resolves-private`, `unresolvable-host`, `reserved-tld`, `single-label-host`, `private-ip`, `non-http-scheme`, `anon_install_id`, `DO_NOT_TRACK`, `CTP_TELEMETRY`, `opt-out`, `disclosed`, `public-url-classified-reportable`, `non-public-sources-suppressed`, `emit-reports-public-and-discloses`, `opt-out-suppresses-all-sending`, `file-source-stays-silent`, `source-url-registered`, `fails-open`. 5 specs (`cl564-refresh-64..68`). Suite 5104→5109.

## §34. Telemetry collector, anonymous project identity, and software-success measurement (v1.28 DESIGN amendment — operator-directed)

**STATUS: DESIGN — backlogged for build.** Persisted per operator instruction to store the strategy as architecture ahead of implementation (mirrors the §31 v1.22 DESIGN-amendment precedent). Full design: [docs/design/v1.28-telemetry-collector-and-success-measurement.md](design/v1.28-telemetry-collector-and-success-measurement.md). Built foundation it extends: §33 CL-564.n (disclosed public-only source-URL telemetry) + `docs/telemetry.md`.

The vision: the plugins can be free/open-source; the durable value is the **data signal** — which authoritative rule/standard **source URLs** correlate with the most **excellent** and most **successful** software — fed back to make GCTP/CTP self-improving. The design's spine is a **tiered consent model** keyed to data sensitivity, because the flywheel only turns if contributors trust it:

- **Tier 0 — public source URLs.** Low sensitivity (public facts). Disclosed, on-by-default, opt-out. **BUILT** (§33 CL-564.n).
- **Tier 1 — anonymous project id + aggregate engineering-health metrics** (DORA: deploy-freq / lead-time / change-fail / MTTR via `security-posture.sh` + C-24 scoreboard; plus test/rule-pass/coverage/fitness-trend). Numbers only, no code. Disclosed, on-by-default, opt-out. DESIGN.
- **Tier 2a — production / in-the-wild success signals** (uptime, adoption, deploy outcomes). High sensitivity (their running product). **Opt-in + barter** (a perk/feature in return), beacon offered at scaffold time. DESIGN.
- **Tier 2b — codebase-signature analysis** (structure/quality patterns of their source). Highest sensitivity. **Opt-in + barter**, separate toggle. DESIGN.

Binding principles: disclosure always (anonymity ≠ exemption from disclosure); opt-in for anything private; barter not coercion (base product fully functional without opting in); **no rights transfer** (permission to collect, never ownership/license over code or product); anonymity by construction (random per-project UUID join key — never derived from repo path/remote — rotatable + deletable); license-clean + commercially distributable.

**Anonymous project identity:** a random UUID generated once per project (`.claude-tdd-pro/project-id`, gitignored by default), an opaque join key linking a project's URL set to its health/success signals without revealing identity.

**Collector infrastructure:** GitHub is explicitly NOT the store (no ingestion endpoint). AWS serverless delivers every resiliency property as a managed feature: API Gateway → Lambda (load-balancing + autoscaling), Firehose → **S3 data lake** partitioned by date (11-nines durability, cross-region replication, versioning-as-backup, Athena SQL for "collect now / mine later"), and/or **DynamoDB** (on-demand auto-scale, automatic partitioning = sharding, PITR = continuous backup, Global Tables = multi-region replication). Delivered as one-command Terraform/SAM **through this repo's own governed IaC pipeline** (dogfooding); the operator deploys with their creds and sets `CTP_TELEMETRY_ENDPOINT`. The §33 CL-564.n client already fails open when the endpoint is unreachable.

**Success measurement (three signals):** (1) **gold-standard reference-corpus benchmarking** — compare plugin-built code to expert-verified excellent codebases + their expert reviews (candidate corpus: the **O'Reilly Architectural Katas** finalist submissions + judge reviews — VERIFY availability + **licensing** online before ingest, O-1; derive non-expressive metrics where license is unclear) to extract a reusable **excellence-signature vector**, then score new code by similarity (a number ⇒ Tier 1; deep per-file analysis ⇒ Tier 2b); (2) **local engineering-health metrics** (§5.2, already computed); (3) **production / in-the-wild success** (§5.3, Tier 2a opt-in). Correlating URL sets ↔ excellence signatures ↔ production success, fed back into source-registry promotion + rule generation, makes GCTP/CTP self-improving. Honesty caveat: these are correlations (self-selection possible) — prefer within-project before/after deltas for causal signal.

Open questions (O-1 kata licensing; O-2 production-signal set + beacon; O-3 correlation-vs-causation method; O-4 EU consent posture; O-5 retention/deletion) and the build roadmap (Tier-1 client → collector IaC → excellence-signature model → Tier-2a beacon+barter → Tier-2b → feedback loop) are enumerated in the design doc. No new §2.X contract; no code in this amendment (DESIGN only). §25 fidelity vocabulary additions: `tiered-consent`, `barter`, `opt-in-barter`, `excellence-signature`, `gold-standard-corpus`, `reference-corpus`, `architectural-kata`, `production-beacon`, `self-improving-flywheel`, `anon-project-id`, `data-lake`, `collector`, `feedback-loop`.

**§34 continuation (v1.28 DESIGN refinement, operator-directed) — consent-as-feature + the meta-improvement engine.** Two refinements added to [docs/design/v1.28](design/v1.28-telemetry-collector-and-success-measurement.md) §§9–10. (1) **Consent-as-feature (§9):** the Tier-2a exchange is reframed from "give data, get an unrelated perk" to **structural reciprocity — the opt-in IS the functionality.** The benchmarking capability exists *because* participants share, so contributing and benefiting are the same act: opting in instantly shows a project its standing within the GCTP/CTP fleet (percentile per excellence/health/production dimension, nearest anonymized exemplars, gap-to-gold-standard, "do X to move up" guidance from the higher-ranked cohort). A **data network effect** compounds it (more opt-ins → sharper benchmark → stronger reason to opt in), which is the moat even with the code free — the *aggregate* is the asset. Guardrails: genuine disclosed exchange, base product fully functional for non-participants (never a dark pattern), anonymity preserved, and **no comparative statistic below a k-anonymity floor** (never a cohort of one), no rights transfer. (2) **Meta-improvement engine (§10):** correlation → reliable deduction → automated, human-gated product + process improvement, and improving that itself. An **inference ladder** (correlation → within-project before/after deltas → matched cohorts → interrupted time series → deliberate canary/holdout/A-B experiments + uplift) raises each claim's evidence rung, effect size, and n; an **action layer** turns validated patterns into PR-gated (§31.2) / review-queue / self-healing (TICKET-008) proposals with evidence attached (never silent standard mutation); a **meta-learning** layer measures each change's downstream effect, tracks an improvement hit-rate, and reallocates toward high-yield intervention types (GCTP/CTP get better at getting better). Named pathologies + mitigations: Goodhart's law (rotate/triangulate metrics), self-selection/survivorship (within-project deltas + matched cohorts), feedback echo chamber (exploration budget re-tests deprecated sources), aggregate re-identification (k-anonymity + differential privacy), single-ecosystem overfit (segment analysis), reproducibility (versioned model + dated snapshots). Adds open questions O-6 (min evidence rung to draft an action), O-7 (explore/exploit budget), O-8 (k-anonymity + DP parameters). §25 fidelity vocabulary additions: `consent-as-feature`, `data-network-effect`, `reciprocity`, `k-anonymity`, `differential-privacy`, `inference-ladder`, `within-project-delta`, `interrupted-time-series`, `uplift`, `meta-learning`, `improvement-hit-rate`, `goodharts-law`, `exploration-budget`, `echo-chamber`, `survivorship-bias`.

**§34 build-location determination (v1.28 §11, operator-directed).** Each roadmap item is assigned a build repo per the prime directive (CTP = capability/standards provider; GCTP = orchestration/scaffolding/user-facing consumer) + joint-design regime. Summary: **CTP** — Tier-0 telemetry (BUILT), Tier-1 anon project-id + health emit, Tier-2b codebase analysis, and the CTP halves of the joint items (signature scoring, beacon data-contract, standards/registry action layer). **GCTP** — scaffold-time beacon injection, user-facing benchmarking presentation, self-healing/HITL orchestration. **Author-infra** (generated via the governed IaC pipeline, operator-deployed, never a consumed plugin surface) — the collector (API GW → Lambda → S3/DynamoDB) + the fleet inference/meta-learning pipeline. Joint items (excellence-signature model, Tier-2a beacon, feedback loop, benchmarking view, meta-improvement engine) land their CTP half in CTP and GCTP half in GCTP as separate coordinated CLs, never cross-staged. Full table + rationale: [docs/design/v1.28 §11](design/v1.28-telemetry-collector-and-success-measurement.md); GCTP-owned slice mirrored in the GCTP repo at `docs/backlog-telemetry-success-measurement.md`.

## §35. Repo-wide `audit` command + total rule enforcement (v1.29 DESIGN amendment — operator-directed)

**STATUS: DESIGN — backlogged for build.** Full design: [docs/design/v1.29-repo-wide-audit-and-total-enforcement.md](design/v1.29-repo-wide-audit-and-total-enforcement.md). Grounded in this session's whole-repo enforcement assessment.

**Invariant:** every file has every rule its four-axis tags mark applicable actually enforced (native OR 3rd-party) at write-time, generation-time, AND audit-time — zero silent skips, zero vacuous greens. Makes §28.57 (universal native enforcer) + §29.6 (one byte-identical path) real and total.

**Answer to "what checks rules today — for tagging?":** today's standing gates (`classify-path --audit` unpathed=0, `config-sync --check`, `source-citations`, `applicable-rules`/`rules-verified`) check rule **tagging / metadata completeness**, NOT per-file enforcement coverage. No gate verifies every file had its applicable rules enforced — this design adds that.

**The `audit` command (centerpiece):** one command, runnable on the **entire repo** (default) or any set of folders/filenames/**glob patterns** (ESLint / `node-glob` syntax: `*` = zero-or-more chars in a segment; `?` = exactly one char; `**` = recursive dirs; `!` = negation/exclude). Flow: accept include patterns + optional exclude patterns → expand against the filesystem → apply includes then subtract excludes → flat list of absolute paths → per file compute four-axis applicability and enforce **every applicable rule** (native + routed tool + native fallback when a tool is absent, §28.56/§28.57), folding verdicts; **fails on `red>0 || not_enforced>0`.** **Integration-test-first (operator-directed):** when auditing the whole repo, audit looks for integration tests FIRST; if absent or failing it must **establish** them, **leverage** them to drive test-first generation of excellent code, and **pin** them against regression (composes with TDD-Guard/regression-pin). **Generation-time total enforcement:** when the plugin generates content, all applicable rules are enforced so generated content conforms (extends §28.60 govern-before-write to the total applicable set).

**Prerequisite enforcement-correctness fixes (from the assessment):** (a) app-code native enforcement (`--include-app-code`) is passed only by `enforce-write-time.sh` — add it at the three leaking native paths (`enforce-standards-on-save.sh`, `composite-audit.sh`, `composite-dispatch.sh native_fallback()`); (b) IaC-tagged rules have NO native glob path in `enforce-file.sh` — add an `iac_aliases`→glob mapping; (c) widen the write-time extension allowlist to the audit file-universe and make `not_enforced` a surfaced/blocking condition. **Shell scripts** (`.sh`, 445 files, the largest kind, zero rules today): tooling research recorded (ShellCheck + shfmt; npm wrappers; source rules from ShellCheck wiki / Google Shell Style Guide) — build **DEFERRED** per operator.

**Build sequence:** CL1 enforcement-correctness fix → CL2 `audit` command (glob + repo-wide + CI gate) → CL3 integration-test-first (establish + pin) → CL4 generation-time total enforcement → CL5 shell enforcement (deferred). No new §2.X contract; DESIGN only (no code in this amendment). §25 fidelity vocabulary additions: `audit-command`, `total-enforcement`, `node-glob`, `glob-targeting`, `include-pattern`, `exclude-pattern`, `integration-test-first`, `establish-tests`, `regression-pin`, `generation-time-enforcement`, `enforcement-coverage`, `iac-native-glob`, `not-enforced-blocking`, `shellcheck`, `vacuous-green`.

**§35 CL1 — BUILT (enforcement-correctness fix; §5 of the design).** Landed the prerequisite native-enforcement fix so app-code AND IaC/cloud/DevOps rules are natively enforced at every phase, not just when a 3rd-party tool is present. `rubric/enforce-file.sh`: added `hcl→*.tf` to `LING2EXT` and an `IAC2EXT` `iac_dialects`→glob map (terraform, cloudformation, kubernetes, helm, ansible, openapi, github_actions, dockerfile, bicep, arm), captured `iac_dialects` per rule, and derive the applicability glob from the IaC axis too (gated on `--include-app-code`, like the language axis) — closing the gap where IaC-tagged rules had NO native glob path and were enforced only by a routed tool (silently unenforced when absent). The three leaking native call sites now pass `--include-app-code`: `hooks/scripts/enforce-standards-on-save.sh`, `rubric/composite-audit.sh`, and `rubric/composite-dispatch.sh` `native_fallback()`. `composite-audit.sh` + `composite-dispatch.sh` also gained `--project <id>` passthrough to `enforce-file.sh`, so a consumer's project-overlay (ESLint-model) scraped rules are enforced at audit-time and in the tool-absent fallback. Verified: a scraped terraform rule natively selects on `.tf` under `--include-app-code` (rules_checked +1), gated off without it; zero regressions across the enforcement spec families (cl485/486/487/491/495/510/524/525/543/544/545/559/564-refresh — identical failing set vs baseline). 3 specs (`cl564-enforce-01..03`): iac-rule-natively-selectable · three-native-paths-include-app-code · audit-dispatch-thread-project. §25 fidelity vocabulary additions: `include-app-code`, `iac_dialects`, `IAC2EXT`, `LING2EXT`, `hcl`, `native-fallback`, `project-overlay`, `natively-selectable`.
