...
KNOWN_NAMESPACES=(google us-government european-union finance-industry owasp w3c web-vitals react node typescript slsa linux-foundation industry-self-regulatory aws azure gcp hashicorp security-governance documentation md yaml json k8s helm compose gha glci azdo circleci bbp jenkins ansible cfn oas gitops observability mesh iac-linter jsonschema iam sbom sarif jwt arch _universal _operator _community _meta)
