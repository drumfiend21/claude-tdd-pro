#!/usr/bin/env bash
# commands/gcp-boundary.sh - S-44 GCP platform boundary (v1.14 §27.16; safety +
# normalization contracts §27.19).
#
# The GCP expert boundary. Consumes the S-41 platform-handoff (platform must be
# gcp), normalizes an INJECTED Google Cloud Recommender response into the same
...
#
# CLI: identical surface to aws-boundary.sh / azure-boundary.sh.
#   --handoff <json>          S-41 platform-handoff.json (platform must be gcp)
#   --native-response <json>  injected GCP Recommender response to normalize
#   --apply                   authorize a mutating external call (default: off)
#   --catalog / --eng-catalog grounding catalogs (default resolved)
#   --out <json>              default standards/gcp-boundary-response.json
#   --now <iso> / --dry-run
#
# stderr: boundary=gcp validated=<true|false> normalized_recommendations=<n>
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help) echo "Usage: gcp-boundary.sh --handoff <json> [--native-response <json>] [--apply] [--out <path>] [--dry-run]" >&2; exit 0 ;;
    *) echo "gcp-boundary: unknown arg: $1" >&2; exit 2 ;;
  esac
done

if [ -z "$HANDOFF" ]; then echo "gcp-boundary: --handoff <json> is required" >&2; exit 2; fi
if [ ! -f "$HANDOFF" ]; then echo "gcp-boundary: handoff not found: $HANDOFF" >&2; exit 2; fi
...
if [ -z "$OUT" ]; then OUT="standards/gcp-boundary-response.json"; fi
...
OUT="$OUT" NOW="$NOW" DRY_RUN="$DRY_RUN" ruby -ryaml -rjson -e '
  ...
  if reason
    ...
    STDERR.puts "boundary=gcp"
    STDERR.puts "validated=false"
    STDERR.puts "validation_failed=#{reason}"
    exit 2
  end

  # ---- Normalize the injected GCP Recommender response (§27.19 Contract B) ----
  grounded = {}
  ...
  MAP = [
    ...
    [%w[availability multi-az multi_az zone region], "multi_az", "aws-reliability-pillar"],
    ...
    [%w[cost rightsiz spend idle], "rightsizing", "finops-framework"]
  ]
  # GCP Recommender primaryImpact.category -> Well-Architected pillar.
  CAT = {"SECURITY"=>"security","RELIABILITY"=>"reliability","COST"=>"cost-optimization",
         "PERFORMANCE"=>"performance-efficiency","MANAGEABILITY"=>"operational-excellence",
  ...
  unless native.empty?
    ...
    if nr.is_a?(Hash)
      native_review_ref = nr["review_ref"] || nr["name"]
      items = nr["recommendations"] || []
      items.each do |r|
        pi = r["primaryImpact"] || {}
        category = pi["category"] || r["category"]
        title = r["description"] || r["title"] || r["recommenderSubtype"]
        ...
        recs << {
          ...
          "title"         => title,
          "pillar"        => pillar,
          "severity"      => (r["priority"] || r["severity"] || "unset"),
