...
### The architecture is APPEND-ONLY — amend by reference, never alter existing content

**Evolve the constitution additively.** When introducing new surface, amendments, designs, or decisions: write the detailed content into a NEW file (e.g. `docs/design/<version>-<slug>.md`), then APPEND a reference block as a new end-of-file section (e.g. `## §27. …`) to `docs/architecture-v1.9.md`. **Never delete or alter any existing §1–§N content.** This is exactly how §23 (v1.9.1), §24 (v1.10), §25 (v1.9.2), §26 (v1.11), and §27 (v1.12) were added. Append-only is the strongest form of "never drift": if no existing bytes change, no prior contract/ID/decision can be silently mutated.

The appended reference block MUST carry inline (because `CLAUDE.md` requires these extractable from the constitution itself): the authoritative feature IDs + §2.X contract numbers, standard-form `- **X-N**` bullets (for `^- \*\*[A-Z]-` grep traversal), the §25 fidelity-audit vocabulary additions, the anti-drift exact pending-folder-name map, and the §20 sequencing note. The bulky rationale, mapping tables, spec sketches, and ticket plan stay in the linked design file. Verify additivity before commit: `git diff --numstat docs/architecture-v1.9.md` must show `0` in the deletions column. See [docs/memory/feedback-additive-amendment-by-reference.md](docs/memory/feedback-additive-amendment-by-reference.md) for the full discipline and checklist. **Apply this to all new and ongoing work touching any constitution-class doc** (the architecture, this `CLAUDE.md`, the PRIMARY RULESET, `MEMORY.md`).

## WORK PRESERVATION IS LAW — commit AND push at every sub-boundary

This project runs in an ephemeral remote-execution container that can be re-cloned between turns, silently resetting the local tree to the last-pushed remote state (observed twice on 2026-06-08). **The remote branch is the only durable store.** Therefore, always and without fail: push every commit immediately (commit+push is one inseparable step); checkpoint-commit at each sub-boundary within a CL (specs-fidelity-clean → substrate-probe-green → full-suite-green), not only at CL completion; never end a turn with uncommitted non-trivial work; and `git fetch` + `git merge --ff-only origin/<branch>` at the start of every working turn before assuming local == remote. The stop-hook "uncommitted changes" message is a hard prompt to commit+push. Full discipline and rationale: [docs/memory/feedback-commit-push-cadence-work-preservation.md](docs/memory/feedback-commit-push-cadence-work-preservation.md). This is standing authorization to push work-preserving checkpoints without a separate approval prompt; the CL-completion commit still carries the Step-4 audit body.
...
- [docs/memory/feedback-additive-amendment-by-reference.md](docs/memory/feedback-additive-amendment-by-reference.md) — **read BEFORE touching any constitution-class doc** (`docs/architecture-v1.9.md`, this `CLAUDE.md`, the PRIMARY RULESET, `MEMORY.md`): the append-only / amend-by-reference discipline. Write detail into a NEW file; APPEND a reference block (IDs + standard-form bullets + §25 vocab + anti-drift folder map + §20 sequencing inline); never delete or alter existing content; verify `git diff --numstat` shows 0 deletions. Codified 2026-06-08 after §27 (v1.12) landed as 29 insertions / 0 deletions. Apply to all new and ongoing work.
- [docs/memory/feedback-commit-push-cadence-work-preservation.md](docs/memory/feedback-commit-push-cadence-work-preservation.md) — **the remote branch is the only durable store** (ephemeral container re-clones between turns; observed twice 2026-06-08). Push every commit immediately (commit+push is one step); checkpoint-commit at each sub-boundary within a CL (specs-clean → probe-green → suite-green); never end a turn with uncommitted work; `git fetch` + ff-merge at turn start. See also "WORK PRESERVATION IS LAW" above and drift mechanism #7.
- [docs/memory/feedback-cl-build-orchestrator.md](docs/memory/feedback-cl-build-orchestrator.md) — **use BEFORE running the CL-promotion choreography**: `scripts/cl-build.sh <cl-number> <phase> <feature-id> [...]` drives Step 0.5 → Step 3 (§25 fidelity gate → stage with `cl<N>-` prefix → filter-run probe → remove pending on probe-pass / rollback on probe-fail → full-suite verify → emit commit-body skeleton at `/tmp/cl-<N>-body.md`). Preserves every integrity gate; the agent still owns Step 0 architecture extraction, Step 1 spec authoring, and audit-findings fill-in before `git commit`. Works for single-feature and batch CLs (per `feedback-batch-cl-convention.md`). Persisted 2026-06-02 alongside the runner parallel+cache optimization.
...
7. **Un-pushed work lost to container re-clone.** The ephemeral remote-execution container is reclaimed/re-cloned between turns; the local tree resets to the last-pushed remote commit. Work committed-but-not-pushed, or merely edited, is lost with no error and no prompt. **Defense:** "WORK PRESERVATION IS LAW" section above + `docs/memory/feedback-commit-push-cadence-work-preservation.md`; push every commit immediately; checkpoint mid-CL; `git fetch` + ff-merge at turn start. First hit 2026-06-08 (reset the local tree two CLs back, twice in one session; recovered each time only because the work had been pushed).
